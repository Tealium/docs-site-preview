...
url: https://docs-preview.tealium.com/client-side-tags/rocket-fuel-tag/
---
## Prerequisites
...
[Load Rules]() determine when and where to load an instance of this Tag on your site. The **Load on All Pages** rule is the default load rule. To load this Tag on a specific page, create a new load rule with the relevant conditions.
...
For instructions on how to map to a Tag&#39;s destination, see [Mapping Data Sources]() article.

**Before You Begin**, we recommend setting up the [E-Commerce Extension]() for this Tag. The Extension, if configured, automatically sends the necessary product and order details to the appropriate Tag destinations. You also have the option to override the Extension by mappings those destinations in the Mapping toolbox (see table below).
...
1. Add a new [Set Data Value Extension]() in your profile and scope it to this Tag.
