...
url: https://docs-preview.tealium.com/iq-tag-management/save-publish/version-history/
---
On accounts with [platform permissions]() enabled, only users with the Client-Side Versions permission can access this screen. For more information, see [Permission Groups]().
...
Load the selected version. Switching to a different version automatically moves the pin icon next to the title of the version being viewed.  You cannot switch to an [archived version](#archived-versions).
* **Expand/Hide Details**  
...
* **Make a Copy**  
Duplicate the current version. Duplicating a version saves it and adds **Duplicate of...** to the beginning of the version name.  Duplication does not automatically publish. Switch back to the saved version and perform a publish. 
...
* From the **Merge Changes into Current Session** drop-down list, select the version you want to [merge into this session](). Both the incoming and current versions must originate from the same origin or ancestor.
...
* **Tags**: Click **Change History** on the [tag details page]().
* **Extensions**: Click **View Change History** on the [extension details page]().
* **Load Rules**: Click **Change History** on the [load rules details page]().
* **Events**: Click **Change History** on the [event details page]().
...
Merging changes between versions simplifies the process of bringing changes from an older version into the currently published version. You can only merge two versions if they have branched away from the same origin. To learn more, see [Merge into current session]().
