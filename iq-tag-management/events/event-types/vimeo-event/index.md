---
title: Vimeo event
description: The Vimeo event sends tracking data when a visitor interacts with an embedded Vimeo video on a page. You can specify event triggers for when the visitor plays, pauses, or reaches a specific playback milestone, or when the video is buffering.
url: https://docs-preview.tealium.com/iq-tag-management/events/event-types/vimeo-event/
---
## Requirements
...
The Vimeo event can track the following event triggers:
...
|`video_id`| The video ID at Vimeo.| `xWlEk2i9r5Q`| String|
...
|`video_platform`| The name of the platform serving the video.| `vimeo`| String|
...
|`iq_event_id` | The UID of the event listener that sent the event.|  `vimeo_video_events_1` | String |
