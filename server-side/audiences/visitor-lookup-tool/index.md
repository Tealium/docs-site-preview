...
description: This article explains the Visitor Lookup tool, which is a graphical interface of the Visitor Lookup API. A visitor lookup uses a visitor ID attribute and a search value to locate a visitor record in the system.
url: https://docs-preview.tealium.com/server-side/audiences/visitor-lookup-tool/
---
...
1. Go to **Server-Side Tools &gt; Visitor** **Lookup**.
...
The transaction ID can be used in the [Visitor Lookup API](/api/v3/visitor-privacy/endpoints/) to check the status of the request.

## REST API

The **REST API** tab provides an interface to the [Visitor Lookup API](/api/v3/visitor-privacy/endpoints/). The API calls are pre-formatted for you as examples of how to implement the same functionality in your application. Select the API call you want to make and adjust the placeholder values in the request URL.
