...
url: https://docs-preview.tealium.com/server-side/data-sources/visitor-identification/
---
...
When you upload your offline customer data using the file import or Snowflake data sources, each imported row is processed as an event. To stitch your imported data with other sources such as web, mobile, or HTTP API, ensure that every row in your data source has a column with a unique visitor ID that associates the data to a customer.  
 ...
 For more information, see [About Visitor Stitching](). 
...
For more information about visitor IDs, see [Anonymous IDs, User Identifiers, and Visitor ID Attributes]().
...
* The uploaded file may have incorrect values in the visitor identification columns. If you cannot filter data to ensure incorrect values are removed prior to uploading to Tealium, disable Visitor ID Mapping in AudienceStream and set up visitor ID enrichments manually.  If your files contain incorrect visitor ID values and visitor ID mapping is enabled, it will not be possible to validate the visitor ID values and may cause errors in visitor stitching. 
